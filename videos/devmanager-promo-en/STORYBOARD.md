...
language: English (on-screen text, no VO)
...
Scene 1 (0.0–1.1s): 米白底；大字"6 terminals to juggle"居中 **kinetic-beat-slam**(`kinetic-beat-slam`) 砸入、`power3` 收；右上一枚虚化 menubar 幽灵 **selective-blur**(`depth-of-field-blur`) 低透明衬底。Centered，主体 ≥45%。
Scene 2 (1.1–2.1s): 速度匹配硬切换字"Which one's still running?"(**hard-cut word-swap** `discrete-text-sequence`，seam=cut-the-curve)。
Scene 3 (2.1–3.1s): 再硬切"Port clash — who's on it?"(`discrete-text-sequence`)。
Scene 4 (3.1–4.0s): 三句退场，薄荷绿收束句"Local dev needs a control room"**spring-pop**(`spring-pop-entrance`) 弹入居中并定住；至多低幅 **subtle jitter**(`sine-wave-loop`)。
...
Scene 2 (1.4–2.8s): 副标"All your local servers, one screen"在标题下 **per-word 揭示**(`dynamic-content-sequencing`)。
...
Scene 2 (1.2–2.8s): **zoom-to-target**(`coordinate-target-zoom`) 推近日志区，窗口其余 **selective-blur**(`depth-of-field-blur`) 压暗；薄荷绿圆点+细引线+胶囊"Live color logs"**spring-pop**(`spring-pop-entrance`)。
Scene 3 (2.8–4.4s): 镜头平移(`viewport-change`)到指标行；胶囊"CPU · Memory · Uptime"揭示；内存数字轻 **count-up**(`counting-dynamic-scale`)。
Scene 4 (4.4–6.0s): 平移变焦到二维码(`coordinate-target-zoom`)；胶囊"LAN QR · scan to debug"揭示；二维码后 **glow bloom**(`ambient-glow-bloom`)。
Scene 5 (6.0–7.2s): 平移到控制按钮；停止/启动键做一次 **press**(`press-release-spring`)；胶囊"One-tap start · restart"揭示。
...
Scene 1 (0.0–1.0s): new-process 弹窗居中(~60%)入场；上方标题"Folder + command = a project"**type-on**(`discrete-text-sequence` + `context-sensitive-cursor`)。
Scene 2 (1.0–2.2s): 字段按序逐字填:目录 → 命令 `npm run dev` → tag(各 `discrete-text-sequence`);命令框一道薄荷绿 **highlight**(`css-marker-patterns`) 扫过。
Scene 3 (2.2–3.4s): 弹窗 **scale-swap**(`scale-swap-transition`) 收成侧栏(main-empty)里的一条项目；第二句"Group by tag · launch a set"per-word 揭示。
...
Scene 3 (2.6–3.4s): "Your local dev, in numbers"在下方(避开底部字幕带)per-word 揭示。
...
Scene 3 (2.6–4.4s): 卡片重组为网络——左节点"AI · Claude / Codex / Cursor"，右簇"Your dev processes"，中间 **connector 自绘**(`svg-path-draw`) + 薄荷绿脉冲沿线走(`avatar-cloud-network`)。
Scene 4 (4.4–5.4s): 右侧项目圆点由灰转薄荷绿依次点亮=被启动(`dynamic-content-sequencing`);"One command — let AI run your processes"揭示。
...
Scene 3 (3.2–4.4s): 大字「DevManager」在标下 **spring-pop**(`spring-pop-entrance`);副"All your local servers, one screen"per-word 揭示(`dynamic-content-sequencing`)。
